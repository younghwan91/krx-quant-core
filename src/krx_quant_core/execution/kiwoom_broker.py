...
def _norm_ord_no(value: Any) -> str:
    """주문번호를 선행 0 없이 정규화한다(``"0000123"`` → ``"123"``).

    누적 체결수량을 주문번호별로 추적하는 내부 키로만 쓴다 — ``Fill.ord_no``
    등 호출부에 돌려주는 값은 원본 문자열 그대로 둔다. 전부 ``"0"`` 인
    입력(``"0000000"``)은 지워지지 않게 원본을 돌려준다.
    """
