...
swing-it ``diagnostics/fragility.py`` 를 그대로 옮겼다(수치 동일).

트레이더가 실제 겪는 시퀀스는 '평균'이 아니다. 괴물(상위 몇 건)이 총이익의 대부분을
만들고, 그 몇 건을 빼면 엣지가 사라진다면 — 그 엣지는 재현성이 낮은 운에 가깝다.
이 모듈은 그 취약성을 개별 트레이드 **R-멀티플(또는 건당 수익) 분포** 관점에서만 잰다.
포트폴리오 프레이밍(슬롯·동시보유·연환산·회전율)은 여기 없다 — 오직 표본 분포.

모든 함수는 순수 배열만 받는다. 스캘핑 건당 수익률 배열에도 그대로 쓸 수 있다.
"""
...
def max_loss_streak(
    R: np.ndarray, *, entry: np.ndarray | None = None, loss_thr: float = 0.0
) -> int:
...
def median_trade(R: np.ndarray) -> float:
    """중앙값 트레이드 — 트레이더가 '보통' 겪는 것. 평균이 꼬리에 끌려간 것과 대비."""
    R = np.asarray(R, float)
    R = R[np.isfinite(R)]
