"""stats.trials — swing-it ``tests/test_trials_ledger.py`` 이식(``logs_dir`` 필수화 반영).

시행 수를 사람 기억이 아니라 파일에서 읽는다.
...
from krx_quant_core.stats.trials import (
    config_fingerprint,
    count_trials,
    ledger_path,
    read_trials,
...
def test_logs_dir_is_required():
    """설치된 패키지 안에서 레포 루트를 추정하면 site-packages 에 원장이 쌓인다 — 경로는 필수."""
    import pytest

    with pytest.raises(TypeError):
        count_trials("x")  # type: ignore[call-arg]
