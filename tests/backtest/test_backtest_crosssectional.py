"""backtest.crosssectional — swing-it ``tests/test_engine_sim_cross.py`` 이식.

Drives the engine functions directly on synthetic ``code × date`` numpy panels
: higher-signal codes drift up, so a
...
def test_staggered_delisting_exit_realises_loss():
    """보유 중 폐지는 기본값에선 조용히 빠지고, delisting_exit 면 마지막 종가로 실현된다."""
    C, V, sig, dates = _panels()
    C = C.copy()
    top = int(np.argmax(sig[:, 0]))
    C[top, 61:70] = C[top, 60] * 0.5  # 61일부터 반토막(정리매매)
    C[top, 70:] = np.nan              # 70일에 폐지 — t=60 트랜치의 청산일(80)엔 가격 없음
    p0, _ = staggered_tranche_backtest(C, V, sig, dates, horizon=60, step=20, top_n=5,
                                       adv_floor=0, start_index=40)
    p1, _ = staggered_tranche_backtest(C, V, sig, dates, horizon=60, step=20, top_n=5,
                                       adv_floor=0, start_index=40, delisting_exit=True)
    assert {"book", "bench", "n_universe"} <= set(p1.columns)
    assert p1["book"].sum() < p0["book"].sum()
